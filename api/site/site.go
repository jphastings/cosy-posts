...
func NewHandler(contentDir, csvPath, siteName string, cacheTTLSeconds int) (*Handler, error) {
	...
	icons := map[string]template.HTML{
		...
		"bell-off":   template.HTML(bellOffSVG),
		"bell-ring":  template.HTML(bellRingSVG),
		"rss":        template.HTML(rssSVG),
...
func (h *Handler) loadPosts(members map[string]auth.Member, prefLang string) []Post {
	var posts []Post
...
func (h *Handler) parsePost(indexPath string, members map[string]auth.Member) (Post, error) {
